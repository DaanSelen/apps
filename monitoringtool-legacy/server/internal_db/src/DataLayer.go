...
func initDBConnection() {
	...
	entry.Query("CREATE TABLE IF NOT EXISTS entry (id INTEGER PRIMARY KEY AUTO_INCREMENT, hostname TEXT NOT NULL, ipaddress TEXT NOT NULL, comp TEXT NOT NULL, time TEXT NOT NULL)")
...
func insertEntry(alert Alert) {
	entry.Query("INSERT INTO entry(hostname, ipaddress, comp, time) VALUES('" + alert.Hostname + "', '" + alert.IpAddr + "', '" + alert.Comp + "', '" + alert.Time + "')")
}

func dataCheck(component string) []Alert {
	var alerts []Alert
	rows, _ := entry.Query("SELECT * FROM entry WHERE comp = '" + component + "'")
	defer rows.Close()
	for rows.Next() {
		var alert Alert
